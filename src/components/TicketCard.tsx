...
const TicketCard: React.FC<TicketCardProps> = ({
  ...
  return (
    <div
      className={cn(
        "p-2 pb-8 w-60 ticket_mask bg-yellow-300 rounded shadow-lg shadow-black",
        { "shadow-2xl shadow-orange-800": selected }
      )}
    >
        ...
        <ul className="text-sm ">
          ...
          {nChildren > 0 && (
            <li className="flex justify-between">
              ...
              <span>€{(ticketPackage.priceChild * nChildren).toFixed(2)}</span>
            </li>
          )}
        </ul>
      </div>
